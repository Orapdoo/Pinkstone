...
public class DefaultChunkRenderer extends ShaderChunkRenderer {
    ...
    private static void addLocalIndexedDrawCommands(MultiDrawBatch batch, long pMeshData, int mask) {
        ...
        int size = batch.size;
        int maxElementCount = batch.maxElementCount;
        ...
        for (int facing = 0; facing < ModelQuadFacing.COUNT; facing++) {
            ...
            final int elementCountInt = UInt32.uncheckedDowncast(elementCount);

            MemoryIntrinsics.putInt(pElementCount + (size << 2), elementCountInt);
            ...
            int visible = (mask >> facing) & 1;
            if (visible == 1 && elementCountInt > maxElementCount) {
                maxElementCount = elementCountInt;
            }
        ...
        for (int i = 0; i <= ModelQuadFacing.COUNT; i++) {
            ...
            if (maskBit == 0) {
                if (lastMaskBit == 1) {
                    ...
                    int elementCountInt = UInt32.uncheckedDowncast((groupVertexCount >> 2) * 6);
                    MemoryIntrinsics.putInt(pElementCount + (size << 2), elementCountInt);
                    MemoryIntrinsics.putInt(pBaseVertex + (size << 2), UInt32.uncheckedDowncast(baseVertex));
                    MemoryIntrinsics.putAddress(pElementPointer + (size << Pointer.POINTER_SHIFT), elementOffsetBytes);
                    if (elementCountInt > maxElementCount) {
                        maxElementCount = elementCountInt;
                    }
                    size++;
