...
public class DynamicTopoData extends DynamicData {
    ...
    private static final ThreadLocal<DistanceSortScratch> DISTANCE_SORT_SCRATCH = ThreadLocal.withInitial(DistanceSortScratch::new);
    ...
    static void distanceSortDirect(IntBuffer indexBuffer, TQuad[] quads, Vector3fc cameraPos) {
        ...
        if (quadCount == 1) {
            ...
            var scratch = DISTANCE_SORT_SCRATCH.get();
            scratch.ensureExactSize(quadCount);

            final var keys = scratch.keys;
            final var perm = scratch.perm;
    ...
    private static final class DistanceSortScratch {
        private int[] keys = new int[0];
        private int[] perm = new int[0];

        private void ensureExactSize(int size) {
